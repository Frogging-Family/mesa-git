...
if [ "$_lib32" == "true" ]; then
  makedepends+=('lib32-libdisplay-info' 'lib32-libxml2' 'lib32-libx11' 'lib32-libdrm' 'lib32-libxshmfence'
                'lib32-libxxf86vm' 'lib32-gcc-libs' 'lib32-libelf' 'lib32-libgcrypt' 'lib32-lm_sensors'
                'lib32-libxdamage' 'gcc-multilib' 'lib32-libunwind' 'lib32-libglvnd' 'lib32-libva'
                'lib32-wayland' 'lib32-libxrandr' 'lib32-expat' 'lib32-spirv-llvm-translator'
                'lib32-rust-libs')
fi
...
build () {
    ...
    # gallium_vdpau requires either r300, r600, radeonsi or nouveau + lib32-libvdpau dependency
    if ( cd "$srcdir/$_mesa_srcdir" && ! git merge-base --is-ancestor 4b54277d2e9420e37cdce98b3a09e6cecf87300d HEAD ); then
      if ( ! pacman -Qq lib32-libvdpau &> /dev/null && [ "$_lib32" == "true" ] ); then
        warning "Gallium VDPAU disabled (missing lib32-libvdpau dependency)"
        warning "lib32-libvdpau was removed from multilib repositories, to fulfill this you may need to install an AUR package or a package from unofficial repositories"
    ...
    # Force meson to resolve venus-protocol as a submodule instead of looking for a system install of it
    if ( cd "$srcdir/$_mesa_srcdir" && git merge-base --is-ancestor c7919be1fa9d99a75eb6ec6042fdff6265fe55e5 HEAD ); then
      _force_fallback_for="syn,paste,rustc-hash,venus-protocol"
    else
      _force_fallback_for="syn,paste,rustc-hash"
    fi
    ...
    arch-meson $_mesa_srcdir _build64 \
       ...
       --force-fallback-for=${_force_fallback_for} \
       -D b_ndebug=true \
